                ...
                if flags.get("max") == '1':
                    ...
                    if flags.get("thr") == '1' and id_value in upper_error_limits and id_value in upper_warning_limits:
                        ...
                        if max_speed_value > upper_error_limits[id_value] and max_speed_value <= upper_warning_limits[id_value]:
                            output_text += f"MAX SPEED for ID {id_value} is within both Upper Error Limit({upper_error_limits[id_value]} Mbit/s) and Upper Warning Limit({upper_warning_limits[id_value]} Mbit/s)\n\n"
                        elif max_speed_value <= upper_error_limits[id_value] and max_speed_value > upper_warning_limits[id_value]:
                            output_text += f"MAX SPEED for ID {id_value} crosses Upper Warning Limit({upper_warning_limits[id_value]} Mbit/s) but is within Upper Error Limit({upper_error_limits[id_value]} Mbit/s)\n\n"
                        elif max_speed_value > upper_error_limits[id_value]:
                            output_text += f"MAX SPEED for ID {id_value} exceeds Upper Error Limit({upper_error_limits[id_value]} Mbit/s)\n\n"
                        else:
                            output_text += f"MAX SPEED for ID {id_value} is within Upper Warning Limit({upper_warning_limits[id_value]} Mbit/s)\n\n"
