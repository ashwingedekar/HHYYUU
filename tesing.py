...
api_endpoint_warning = 'https://tp-prtg-101-100.comtelindia.com:10443/api/getobjectproperty.htm?subtype=channel&subid=-1&name=limitmaxwarning&show=nohtmlencode&username=Ashwin.Gedekar&passhash=1815236212'
api_endpoint_error = 'https://tp-prtg-101-100.comtelindia.com:10443/api/getobjectproperty.htm?subtype=channel&subid=-1&name=limitmaxerror&show=nohtmlencode&username=Ashwin.Gedekar&passhash=1815236212'
        ...
        try:
            ...
            if not selected_data.empty:
                if flags.get("max") == '1':
                    ...
                    # Check if the upper error limit and warning limit are available for the current ID
                    if id_value in upper_error_limits and id_value in upper_warning_limits:
                        ...
                        # Check if max speed values for the current ID are within the limits
                        if max_speed_value > upper_error_limits[id_value] and max_speed_value <= upper_warning_limits[id_value]:
                            output_text += f" ALERT !!! MAX SPEED for ID {id_value} crosses Upper Error Limit but is within Upper Warning Limit\n\n"
                        elif max_speed_value <= upper_error_limits[id_value] and max_speed_value > upper_warning_limits[id_value]:
                            output_text += f" ALERT !!! MAX SPEED for ID {id_value} crosses Upper Warning Limit but is within Upper Error Limit\n\n"
                        elif max_speed_value <= upper_error_limits[id_value] and max_speed_value <= upper_warning_limits[id_value]:
                            output_text += f" MAX SPEED for ID {id_value} is within both Upper Error Limit and Upper Warning Limit\n\n"
                        else:
                            output_text += f" MAX SPEED for ID {id_value} crosses both Upper Error Limit and Upper Warning Limit\n\n"
